...
<ParamField path="init" type="ParsedSearchParams">
...
```tsx Setting Parameters
const [searchParams, setSearchParams] = useSearchParams();

// Set directly
setSearchParams({ userId: '123', tab: 'settings' });
...
```tsx Initial Parameters
// Set parameters on mount
useSearchParams({ view: 'grid', sort: 'date' });
```
</CodeGroup>
...
<ParamField path="expiry" type="Date" required>
  Target date to count down to
</ParamField>

<ParamField path="options.interval" type="number" default="1000">
...
<ParamField path="loadingCondition" type="boolean[] | boolean | (() => boolean)" required>
  Loading condition(s) to stabilize
</ParamField>

<ParamField path="options.delay" type="number" default="300">
  Delay in milliseconds before clearing loading state
</ParamField>

<ParamField path="options.initialState" type="boolean" default="true">
  Initial loading state
</ParamField>
...
// True if any are loading, with debounced transition to false
const stableLoading = useStableLoading([loading1, loading2]);
```

```tsx Function Form
const stableLoading = useStableLoading(
  () => loadingState1 || loadingState2,
  { delay: 500 }
);
```
</CodeGroup>
